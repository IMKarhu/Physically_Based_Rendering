...
namespace karhu
{
    GraphicsPipeline::GraphicsPipeline(Vulkan_Device& device)
        :m_VkDevice(device)
    {
    }
	...
	void GraphicsPipeline::createGraphicsPipeline(GraphicsPipelineCreateStruct pipelineStruct)
	{
        ...
        //pipelineStruct.viewport.width = pipelineStruct.viewportWidth; /*(float)m_VkSwapChain->m_SwapChainExtent.width;*/ DONE
        //pipelineStruct.viewport.height = pipelineStruct.viewportheight; /*(float)m_VkSwapChain->m_SwapChainExtent.height;*/ DONE
        pipelineStruct.viewport.minDepth = 0.0f;
        pipelineStruct.viewport.maxDepth = 1.0f;

        pipelineStruct.scissor.offset = { 0, 0 };
        //pipelineStruct.scissor.extent = m_VkSwapChain->m_SwapChainExtent; DONE
        ...
        //pipelineStruct.pipelineLayoutInfo.pSetLayouts = &m_DescriptorLayout; // nullptr if no descriptors used STILL NEED TO FIND A WAY TO GIVE SETLAYOUT
        ...
        VK_CHECK(vkCreatePipelineLayout(m_VkDevice.m_Device, &pipelineStruct.pipelineLayoutInfo, nullptr, &m_PipelineLayout));
    ...
    void GraphicsPipeline::createRenderPass(VkFormat& swapChainImageFormat)
    {
        ...
        colorAttachment.format = swapChainImageFormat, /*m_VkSwapChain->m_SwapChainImageFormat;*/
    ...
    VkFormat GraphicsPipeline::findSupportedFormat(const std::vector<VkFormat>& formats, VkImageTiling tiling, VkFormatFeatureFlags features)
    {
