...
namespace engine
{
    ...
    TinyRenderer::~TinyRenderer()
    {
        ...
        vkDestroyDescriptorSetLayout(m_Device.m_Device,
                                     m_DescriptorLayout,
                                     nullptr);

        for (size_t i = 0; i < 2; i++)
        {
            vkDestroySemaphore(m_Device.m_Device,
                               m_Semaphores.availableSemaphores[i],
                               nullptr);
            vkDestroySemaphore(m_Device.m_Device,
                               m_Semaphores.finishedSemaphores[i],
                               nullptr);
    ...
    void TinyRenderer::buildVulkan(uint32_t maxFramesInFlight,
                                   std::vector<karhu::vkglTFModel *>& models)
    {
        setupDebugMessenger();
        ...
        m_Descriptors.addBind(0,
                              VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                              1,
                              VK_SHADER_STAGE_VERTEX_BIT);
        m_Descriptors.addBind(1,
        ...
        /*Depth stuff*/
         models[0]->m_Texture.createTexture("../textures/blue.jpg", m_CommandPool);
        ...
        for (const auto& model : models)
        {
            std::vector<karhu::Vertex> m_Vertices;
                ...
                buffer.createVkBuffer<karhu::Vertex>(m_Vertices,
                                                     model->m_VertexBuffer.m_Buffer,
                                                     model->m_VertexBuffer.m_BufferMemory,
                    VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                                     m_CommandPool);

                buffer.createVkBuffer<uint32_t>(m_Indices,
                                                model->m_IndexBuffer.m_Buffer,
                                                model->m_IndexBuffer.m_BufferMemory,
                    VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                                                m_CommandPool);
        }
        createUniformBuffers(maxFramesInFlight);

        m_DescriptorSets = m_Descriptors.createDescriptorSets(maxFramesInFlight,
                                                              m_DescriptorLayout,
                                                              m_UniformBuffers,
                                                              models[0]->m_Texture);
    ...
    void TinyRenderer::draw(uint32_t currentFrame, karhu::vkglTFModel *model)
    {
        ...
        uint32_t imageIndex;
        VkResult res = vkAcquireNextImageKHR(m_Device.m_Device,
                                             m_SwapChain.m_SwapChain,
                                             UINT64_MAX,
                                             m_Semaphores.availableSemaphores[currentFrame],
                                             VK_NULL_HANDLE,
                                             &imageIndex);
        ...
        VK_CHECK(vkQueueSubmit(m_Device.m_GraphicsQueue,
                               1,
                               &submitinfo,
    ...
    void TinyRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer,
                                           uint32_t index,
                                           karhu::vkglTFModel *model)
    {
    ...
    void TinyRenderer::createDepthResources()
    {
        ...
        texture.createImage(m_SwapChain.m_SwapChainExtent.width,
                            m_SwapChain.m_SwapChainExtent.height,
                            format,
        ...
        m_DepthImageView = texture.createImageview(m_DepthImage,
                                                   format,
